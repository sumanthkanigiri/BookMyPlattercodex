...
Deno.serve(async (request) => {
  ...
  try {
    ...
    return json({ error: error instanceof Error ? error.message : 'Authentication required' }, 401);
  }
  const { code, subtotal } = await readJson<RequestBody>(request);
  if (typeof code !== 'string' || typeof subtotal !== 'number' || !Number.isFinite(subtotal) || subtotal <= 0) {
    return json({ valid: false, discount: 0 }, 400);
  }
