import { adminClient, authenticatedUserId, corsPreflight, json, readJson, sendFirebasePush } from '../_shared.ts';
  ...
  for (const channel of channels) {
    try {
      if (channel === 'push') {
        ...
        await sendFirebasePush(body.userId, title, message, body.orderId ? { orderId: body.orderId } : {});
      } else if (channel === 'sms') {
