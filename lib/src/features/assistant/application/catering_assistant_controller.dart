...
class AiCateringPlan {
  const AiCateringPlan({required this.bestPackage, required this.budget, required this.guestEstimate, required this.servingPlan, required this.festivalRecommendation, required this.supportAnswer, required this.faq});

  factory AiCateringPlan.fromMap(Map<String, dynamic> map) => AiCateringPlan(
        bestPackage: Map<String, dynamic>.from((map['best_package'] as Map?) ?? const {}),
        budget: Map<String, dynamic>.from((map['budget'] as Map?) ?? const {}),
        guestEstimate: Map<String, dynamic>.from((map['guest_estimate'] as Map?) ?? const {}),
        servingPlan: Map<String, dynamic>.from((map['serving_plan'] as Map?) ?? const {}),
        festivalRecommendation: map['festival_recommendation'] as String? ?? '',
        supportAnswer: map['support_answer'] as String? ?? '',
        faq: [for (final item in (map['faq'] as List<dynamic>? ?? const [])) Map<String, dynamic>.from(item as Map)],
      );

  final Map<String, dynamic> bestPackage;
  final Map<String, dynamic> budget;
  final Map<String, dynamic> guestEstimate;
  final Map<String, dynamic> servingPlan;
  final String festivalRecommendation;
  final String supportAnswer;
...
class CateringAssistantRepository {
  ...
  Future<AiCateringPlan> generatePlan(CateringAssistantState state) async {
    final payload = await _client.rpc<dynamic>('generate_ai_catering_plan', params: {
    ...
    return AiCateringPlan.fromMap(Map<String, dynamic>.from(payload as Map));
  }
...
final aiCateringPlanProvider = FutureProvider<AiCateringPlan?>((ref) async {
  final state = ref.watch(cateringAssistantControllerProvider).valueOrNull;
  if (state == null || state.eventType == null || state.foodPreference == null) return null;
  return ref.watch(cateringAssistantRepositoryProvider).generatePlan(state);
});
