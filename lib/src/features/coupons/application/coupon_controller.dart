...
class CouponRepository {
  ...
  Future<Coupon?> bestFor(double subtotal) async {
    if (subtotal <= 0) return null;
    final rows = await _client.rpc<List<dynamic>>(
      'best_customer_coupon',
      params: {'p_subtotal': subtotal},
    );
    if (rows.isEmpty) return null;
    final data = rows.first as Map<String, dynamic>;
    return Coupon.fromValidation(
...
class CouponController extends StateNotifier<AsyncValue<Coupon?>> {
  ...
  Future<void> applyBest(double subtotal) async {
    state = const AsyncLoading();
    state = await AsyncValue.guard(() => _repository.bestFor(subtotal));
  }
